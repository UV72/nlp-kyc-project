...
OCR = PaddleOCR(
    lang="en",
    # new API fields; use_textline_orientation replaces use_angle_cls
    use_textline_orientation=False,
    use_doc_orientation_classify=False,
    use_doc_unwarping=False,
    # device="cpu",  # or "gpu:0" if you have GPU working
)


def ocr_v3_compat(img):
    """
    Wrap PaddleOCR 3.x `predict()` to mimic the old `ocr()` output:
    returns: [ [ [box, (text, score)], ... ] ]  for a single image
    so existing loops over `ocr_result` still work.
    """
    results = OCR.predict(img)   # <-- no cls/det/rec kwargs in v3

    if not results:
        return []

    res_obj = results[0]
    res_json = res_obj.json["res"]   # structured result dict
    ...
    lines = []
    for box, txt, score in zip(rec_boxes, rec_texts, rec_scores):
        # box is typically a 4x2 array of points
        if isinstance(box, np.ndarray):
            box = box.tolist()
        lines.append([box, (txt, float(score))])

    # Match old API: list per image
    return [lines]
...
def process_id(
    ...
    Returns:
        dict: {
            ...
            "fields": { <class_name>: <extracted_text>, ... }
        }
    """
    # ---------- Load image ----------
    ...
    if model_name is None:
        ...
        logger.info(
            f"Detected document type: {doc_type}, confidence: {confidence:.2f}"
        )
        ...
        if model_name is None:
            logger.warning(
                f"No detection model mapped for document type: {doc_type}. "
        ...
        for box in result.boxes:
            try:
                ...
                if cls_idx >= len(class_names):
                    logger.warning(
                        f"Invalid class index {cls_idx} for model {model_name}. "
                        f"Skipping box."
                    )
                    continue
                ...
                logger.info(
                    f"Detected box for class index: {cls_idx}, "
                    f"class name: {class_name}, confidence: {conf:.2f}, "
                    f"coords: {xyxy}"
                )

                if cls_idx not in filtered_boxes or conf > filtered_boxes[cls_idx]["conf"]:
                    filtered_boxes[cls_idx] = {
                        "conf": conf,
                        "xyxy": xyxy,
    ...
    for cls_idx, data in filtered_boxes.items():
        ...
        try:
            ...
            logger.info(
                f"Processing class: {class_name} at "
            ...
            black_canvas = np.ones((h, w, 3), dtype=np.uint8) * 255  # white background
            ...
            black_canvas[
                top_left_y : top_left_y + region_h,
                top_left_x : top_left_x + region_w,
            ] = region_img

            # ---- OCR (v3 wrapper) ----
            ocr_result = ocr_v3_compat(black_canvas)
            ...
            for line in ocr_result:
                ...
                for word_info in line:
                    if (
                        ...
                        continue
                    extracted_text_parts.append(word_info[1][0])
            ...
            # Draw OCR bounding boxes on the canvas
            for line in ocr_result:
                if line is None:
                    continue
                for word_info in line:
                    ...
                    try:
                        ...
                        cv2.rectangle(
                            black_canvas, (x1, y1), (x2, y2),
                            (0, 255, 0), 2
                        )
                    except (IndexError, TypeError) as e:
                        logger.error(
                            f"Error drawing OCR box for class {class_name}: {e}"
                        )
                        continue
            ...
            cv2.rectangle(
                output_image, (x_min, y_min), (x_max, y_max),
                (0, 255, 0), 2
            )
            cv2.putText(
                output_image, class_name,
                (x_min, y_min - 10),
    ...
    logger.info(f"Final detected_text: {detected_text}")
...
def process_id_original(image_path, model_name=None, save_json=True, output_json="detected_text.json", verbose=False, classify_only=False):
    """
    Process an ID image to classify document type, detect fields, and extract text.
    ...
    for cls, data in filtered_boxes.items():
        try:
            ...
            # Perform OCR (PaddleOCR v3.x via compat wrapper)
            ocr_result = ocr_v3_compat(black_canvas)
